...
func ParseTodoResourceName(name string) (*TodoResourceName, error) {
	...
	if listID <= 0 {
		return nil, fmt.Errorf("invalid todo list id: must be > 0")
	}
	...
	if tid <= 0 {
		return nil, fmt.Errorf("invalid todo id: must be > 0")
	}
...
func ParseTodoListResourceName(name string) (*TodoListResourceName, error) {
	// Parse "users/100/todo-lists/200" -> TodoListResourceName{...}
	parts := strings.Split(name, "/")
	if len(parts) != 4 {
